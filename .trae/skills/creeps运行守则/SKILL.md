...
- **物流规则**：
  ...
  - **卸货优先**：到达目的地后，必须卸载货物直到背包为空或目标已满，禁止中途更换目标（除非触发重定向）。
  - **动态响应**：优先服务于积压最严重的矿源（孵化时分配），并根据全局需求（如升级者饥饿）调整优先级。
- **允许**：
  - 捡起掉落的能量。
  - 从墓碑/废墟中提取能量。
- **禁止**：
...
- **物流模式**：就近取货。优先从附近的容器、仓库或掉落资源获取能量。
  - **基地便利取能**：如果在 Spawn/Extension 附近 (Range 5) 且其他来源不足，允许直接从 Spawn/Extension 取能，避免等待。
- **禁止**：
