const structurePlanner = {
    ...
    execute: function(room, analysis) {
        ...
        if (room.energyAvailable < 300 && room.find(FIND_MY_CONSTRUCTION_SITES).length > 0) {
            return;
        }
    ...
    planContainer: function(room, centerPos, range, type, biasPos) {
        ...
        for (let x = centerPos.x - range; x <= centerPos.x + range; x++) {
            for (let y = centerPos.y - range; y <= centerPos.y + range; y++) {
                ...
                if (structures.length > 0 && structures[0].structureType !== STRUCTURE_ROAD) continue;
    ...
    visualize: function(room, analysis) {
        ...
        // 绘制几何中心
        visual.circle(analysis.centerPos, {fill: 'transparent', radius: 0.5, stroke: '#00ffff'});
        visual.text('Target Center', analysis.centerPos.x, analysis.centerPos.y + 0.2, {color: '#00ffff', font: 0.3});

        // 绘制连线
        analysis.sources.forEach(s => {
            visual.line(analysis.spawn.pos, s.pos, {color: '#555555', lineStyle: 'dashed'});
        });
        visual.line(analysis.spawn.pos, analysis.controller.pos, {color: '#555555', lineStyle: 'dashed'});

        // 输出分析报告
        const x = 1;
        const y = 8;
        visual.text(`🏗️ Structure Planner Report`, x, y, {align: 'left', color: '#ffffff'});
        ...
        visual.text(`Source Span: ${analysis.maxSourceSpan}`, x, y+3, {align: 'left', color: '#aaaaaa', font: 0.5});
        
        // 预计效率提升 (模拟数据)
        const efficiency = analysis.isSpawnCentral ? "High (>50%)" : "Normal";
        visual.text(`Est. Efficiency Gain: ${efficiency}`, x, y+4, {align: 'left', color: '#00ff00', font: 0.5});
    }
};
