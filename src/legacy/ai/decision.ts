...
export default class Brain {
  ...
  generateTasks() {
    ...
    // [FIX] Prioritize completing existing structures before starting new ones.
    // Group sites by structure type and progress.
    const sites = StructureCache.getConstructionSites(this.room);
    
    // Sort sites:
    // 1. Critical Structures (Spawn, Extension, Tower) -> High Priority
    // 2. High Progress (finish what you started) -> Medium Priority
    // 3. Others (Roads, Containers) -> Low Priority
    
    sites.forEach((s) => {
        let baseScore = config.LOGISTICS.SCORES.BUILDER;
        
        // Bonus for Critical Structures
        ...
        // Bonus for progress (0 to 20 points)
        // progress / progressTotal * 20
        if (s.progressTotal > 0) {
            baseScore += (s.progress / s.progressTotal) * 20;
        }
        
        // Distance penalty handled in Task.getScore, but we can boost base here.
        
        // Create Task
        const task = new Task("build", s.id, baseScore, {}, ["builder"]);
