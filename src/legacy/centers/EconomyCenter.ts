...
export class EconomyCenter {
  static run(room: Room) {
    ...
    // Update Energy Crisis Level
    EnergyManager.update(room);
  ...
  private static generateTransportTasks(room: Room) {
    ...
    const dropped = room.find(FIND_DROPPED_RESOURCES, {
      filter: (r) => r.resourceType === RESOURCE_ENERGY && r.amount > 50, // Lowered threshold from 100
    });

    dropped.forEach((res) => {
      const taskId = `PICKUP_${res.id}`;
      // [Optimization] Decay Priority
      // If amount is huge (>1000) OR decaying soon (<200 ticks), Critical/High
      // 1000 energy is worth a trip even if far.
      // 50 energy is not worth a dedicated trip if far.
      
      let priority = TaskPriority.NORMAL;
      if (res.amount > 1000) priority = TaskPriority.HIGH;
      
      // Decay logic: Dropped resources decay.
      // Not available in 'res' directly? 'amount' decreases.
      // Actually there is no 'ticksToDecay' on Resource object? 
      // Resource decays 10% or 1 per tick? It's simply `amount`.
      // Tombstones have ticksToDecay.
      // So for dropped, bigger is better.
      
      // If we already have a task, skip?
      // No, update priority if amount changed significantly?
      ...
      GlobalDispatch.registerTask({
        ...
        creationTime: Game.time,
        // [Optimization] Pass amount to sort queues
        data: { resource: RESOURCE_ENERGY, amount: res.amount },
      });
    });
    
    // Tombstones/Ruins
    const ruins = room.find(FIND_RUINS, { filter: r => r.store[RESOURCE_ENERGY] > 50 });
    ruins.forEach(ruin => {
        const taskId = `PICKUP_${ruin.id}`;
        let priority = TaskPriority.NORMAL;
        if (ruin.store[RESOURCE_ENERGY] > 500 || ruin.ticksToDecay < 1000) priority = TaskPriority.HIGH;
        
        GlobalDispatch.registerTask({
            id: taskId, type: TaskType.PICKUP, priority, validRoles: ["hauler"],
            targetId: ruin.id, pos: ruin.pos, maxCreeps: 1, creepsAssigned: [],
            requirements: { bodyParts: [CARRY] }, creationTime: Game.time,
            data: { resource: RESOURCE_ENERGY, amount: ruin.store[RESOURCE_ENERGY] }
        });
    });
  }
  ...
  private static generateActiveDeliveryTasks(room: Room) {
    ...
    workers.forEach((worker) => {
      ...
      // Pre-fetch Delivery:
      // If worker has < 50% energy AND is working (meaning they will need more soon)
      // OR explicitly requesting.
      // But avoid over-delivery if they are nearly full (e.g. ratio > 0.8)
      // Unless it's a critical Upgrader who burns fast.
      
      if (!isRequesting && energyRatio > 0.5) return; 

      // Prevent spamming tasks for the same worker if one already exists
      const taskId = `DELIVER_${worker.id}`;
      if (GlobalDispatch.getTask(taskId)) return;
      ...
      // Critical Builder Request (marked in Builder role)
      if (worker.memory.priorityRequest) priority = TaskPriority.HIGH;

      if (
        worker.memory.role === "upgrader" &&
        room.controller.ticksToDowngrade < 4000
      )
        priority = TaskPriority.CRITICAL; // Boost to CRITICAL if dying
      else if (worker.memory.role === "upgrader" && energyRatio < 0.2) {
         // Upgrader running dry: Boost priority to keep uptime
         priority = Math.max(priority, TaskPriority.HIGH);
      }
