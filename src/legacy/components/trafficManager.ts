...
const TrafficManager = {
  ...
  getTrafficMatrix: function (room, existingMatrix) {
    ...
    allCreeps.forEach((c) => {
      ...
      // [NEW] Read idle ticks from memory (managed by movement module)
      const idleTicks = (c.memory as any)._move?.stuckCount || 0;
  ...
  visualizeTraffic: function (room) {
    ...
    creeps.forEach((creep) => {
      ...
      const idleTicks = (creep.memory as any)._move?.stuckCount || 0;
      if (idleTicks > 2) {
