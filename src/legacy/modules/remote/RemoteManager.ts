...
export class RemoteManager {
  static run(room: Room) {
    ...
    if (Game.cpu.bucket < 100) {
      if (Game.time % 100 === 0)
        console.log(
          `[RemoteManager] Low CPU (< 100), skipping remote management for ${room.name}`,
        );
      return;
    }

    // Force enable log for user confirmation
    if (Game.time % 100 === 0) {
        console.log(`[RemoteManager] Remote Mining System Active for ${room.name}`);
    }
        ...
        if (!Memory.rooms[targetRoom]) {
          ...
          if (!GlobalDispatch.getTask(taskId)) {
            GlobalDispatch.registerTask({
              id: taskId,
              type: TaskType.SCOUT,
              priority: TaskPriority.HIGH, // [FIX] Boost priority to ensure expansion
  ...
  private static evaluateRemote(homeRoom: Room, remoteRoom: Room) {
    ...
    // [NEW] Check Source Keeper Lair
    const lairs = remoteRoom.find(FIND_STRUCTURES, {
        filter: (s) => s.structureType === STRUCTURE_KEEPER_LAIR
    });
    if (lairs.length > 0) {
        // SK Room Logic
        // Requirement: High RCL and strong economy
        if (homeRoom.controller && homeRoom.controller.level >= 7) {
             console.log(`[RemoteManager] ⚔️ Target Acquired: SK Room ${remoteRoom.name}`);
             // Proceed to add
        } else {
             if (Game.time % 100 === 0) console.log(`[RemoteManager] ⚠️ Skipping SK Room ${remoteRoom.name} (Requires RCL 7)`);
             return;
        }
    }
  ...
  private static manageRemoteRoom(homeRoom: Room, remoteName: string) {
    ...
    // [NEW] Branch: Standard vs SK
    const isSK = remoteRoom.find(FIND_STRUCTURES, { filter: s => s.structureType === STRUCTURE_KEEPER_LAIR }).length > 0;
    if (isSK) {
        this.manageSKRoom(homeRoom, remoteName);
        return;
    }
  ...
  private static manageSKRoom(homeRoom: Room, remoteName: string) {
    const remoteRoom = Game.rooms[remoteName];
    ...
    remoteRoom.memory.sources?.forEach((sourceData: any) => {
        const sourceId = sourceData.id;
        const sourcePos = new RoomPosition(sourceData.pos.x, sourceData.pos.y, sourceData.pos.roomName);
        
        // Find Lair
        const lair = sourcePos.findClosestByRange(FIND_STRUCTURES, {
            filter: s => s.structureType === STRUCTURE_KEEPER_LAIR
        }) as StructureKeeperLair;
        
        if (!lair) return;

        // 1. Dispatch Guard (Priority: Critical)
        // One guard per Lair
        const guardId = `sk_guard_${lair.id}`;
        if (!GlobalDispatch.getTask(guardId)) {
            GlobalDispatch.registerTask({
                id: guardId,
                type: TaskType.SK_GUARD,
                priority: TaskPriority.CRITICAL,
                targetId: lair.id,
                pos: lair.pos,
                data: {
                ...
                requirements: { bodyParts: [ATTACK, MOVE, HEAL] }, // Just hint
                creepsAssigned: [],
                maxCreeps: 1,
                validRoles: ['sk_guard'],
                creationTime: Game.time,
                autoRemove: false // Keep active
            });
        }

        // 2. Dispatch Miner (Priority: High)
        const minerId = `sk_mine_${sourceId}`;
        if (!GlobalDispatch.getTask(minerId)) {
            GlobalDispatch.registerTask({
                id: minerId,
                type: TaskType.SK_MINE,
                priority: TaskPriority.HIGH,
                targetId: sourceId,
                pos: sourcePos,
                data: {
                ...
                requirements: { bodyParts: [WORK, CARRY, MOVE] },
                creepsAssigned: [],
                maxCreeps: 1,
                validRoles: ['sk_miner'],
                creationTime: Game.time
        ...
        // 3. Dispatch Hauler (Priority: Normal)
        const haulerId = `sk_haul_${sourceId}`;
        const existingHaulTask = GlobalDispatch.getTask(haulerId);
        
        // Calculate max haulers based on pile
        let maxHaulers = 2;
        const sourceObj = Game.getObjectById(sourceId as Id<Source>);
        if (sourceObj) {
            const pile = sourceObj.pos.findInRange(FIND_DROPPED_RESOURCES, 2, {
            ...
            const amount = pile.reduce((sum, r) => sum + r.amount, 0);
            if (amount > 1500) maxHaulers = 3;
            if (amount > 3000) maxHaulers = 4;
        }

        if (!existingHaulTask) {
             GlobalDispatch.registerTask({
                id: haulerId,
                type: TaskType.SK_HAUL, // Maps to SKHauler
                priority: TaskPriority.NORMAL,
                targetId: sourceId,
                pos: sourcePos,
                data: {
